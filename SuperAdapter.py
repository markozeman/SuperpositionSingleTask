...
if __name__ == '__main__':
    ...
    more_superadapters = True   # if True, we have more than one SuperAdapter for all tasks, e.g. one SuperAdapter for only two tasks
    change_superadapter = 2     # every change_superadapter-th task use new set of adapters in superposition
    ...
    if more_superadapters:
        superadapter_all_results = []
    ...
    for r in range(num_runs):
        ...
        if more_superadapters:
            superadapter_tasks_results = [()] * num_tasks   # list of results per task, every task in the form of (acc, auroc, auprc)
        ...
        contexts, layer_dimension = create_adapter_context_vectors(model, num_tasks, adapter_trainable_layers)
        ...
        for t in range(num_tasks):
            ...
            print('Number of all parameters: ', count_parameters(model))
            ...
            for epoch in range(num_epochs):
                ...
                if do_early_stopping:
                    ...
                    if stopping_criteria == 'acc' and val_acc > early_stopping[0]:   # improvement only on acc
                        ...
                        if more_superadapters:  # in this case acc_e, auroc_e, auprc_e have a different meaning
                            superadapter_tasks_results, acc_e, auroc_e, auprc_e = evaluate_tasks_average_more_SuperAdapters\
                                (model, all_tasks_test_data, contexts, layer_dimension, t, use_MLP, change_superadapter,
                                 superadapter_tasks_results, adapter_trainable_layers=adapter_trainable_layers)
                        else:
                            acc_e, auroc_e, auprc_e = evaluate_results(model, contexts, layer_dimension, all_tasks_test_data,
                                                                       True, t, first_average, use_MLP, batch_size, False, adapter_trainable_layers)
            ...
            #             print(name)
            #             torch.save(params, 'models/bias_task%d_%d.pt' % (t, ind))
            #             ind += 1

            # # save output layer
            # if more_superadapters:
            #     for name, params in model.named_parameters():
            #         if name == "mlp.2.weight":   # output layer
            #             print(name)
            #             torch.save(params, 'models/output_layer_task%d.pt' % t)
            ...
            # perform context multiplication
            if t < num_tasks - 1:   # do not multiply with contexts at the end of last task
                context_multiplication(model, contexts, layer_dimension, t, adapter_trainable_layers)
        ...
        if more_superadapters:
            superadapter_all_results.append(superadapter_tasks_results)
    ...
    if more_superadapters:  # fill 'end_performance' with 'superadapter_all_results' data
        superadapter_all_results = np.array(superadapter_all_results)
        superadapter_all_results_until_th_task = []
        for line in superadapter_all_results:
            superadapter_all_results_until_th_task.append([np.mean(line[:task_id + 1, :], axis=0) if task_id != 0 else tup for task_id, tup in enumerate(line)])

        means = np.mean(superadapter_all_results_until_th_task, axis=0)
        stds = np.std(superadapter_all_results_until_th_task, axis=0)

        for i in range(num_tasks):
            end_performance[i]['acc'] = means[i][0]
            end_performance[i]['auroc'] = means[i][1]
            end_performance[i]['auprc'] = means[i][2]
            end_performance[i]['std_acc'] = stds[i][0]
            end_performance[i]['std_auroc'] = stds[i][1]
            end_performance[i]['std_auprc'] = stds[i][2]

        ### WARNING ###
        # The performance of first 5 tasks is not comparable to other approaches, since it is calculated in a different way:
        # If change_superadapter = 3, the first 3 tasks have the same set of adapters and the last 3 tasks have a different set of adapters.
        # In this case, the results from the first and the second task represent the performance when the same set of adapters is used for 3 tasks,
        # so the performance in 'superadapter_all_results' measures the performance of the first 3 tasks after learning those 3 tasks.
    ...
    plot_multiple_histograms(end_performance, num_tasks, metrics,
                             '#runs: %d, %s task results, %s model, %s' % (num_runs, first_average, 'SuperAdapter',
